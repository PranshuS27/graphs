...
public class SaveMiles {
    ...
    public static void main(String[] args) {
        ...
        StdIn.readLine();
        sm.dijkstra(sm.oldToNew.get(dimMeet), sm.normalizedAdjList.length);
        for (int i = 0; i < numPeople; i++)
        {
            ...
            for(int k = 0; k<dimensions.size(); k++)
            {
                ...
                {
                    dimensionNo = dimensions.get(k).getCurrent();
                }
            }
            if(sm.dist[sm.oldToNew.get(dimensionNo)]*2<=time)
            {
                numActual++;
            }
        }
        StdOut.setFile(args[5]);
        if(numActual>= numNeeded)
        {
            StdOut.print("TRUE");
        }
        else
